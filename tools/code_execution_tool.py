...
def _resolve_child_cwd(
    ...
    - ``project``: the session's cwd override (``session.cwd.set``), then
      TERMINAL_CWD, then ``os.getcwd()``, with staging as a last resort.
    """
    if mode != "project":
        return staging_dir
    # Per-session cwd override (session.cwd.set) — matches write_file/terminal.
    if task_id:
        try:
