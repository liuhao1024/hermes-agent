...
class LocalEnvironment(BaseEnvironment):
    ...
    def _update_cwd(self, result: dict):
        """Extract CWD from the in-band stdout marker — no redundant temp-file read.

        The command wrapper emits the same ``pwd -P`` value both to a temp
        file (``base.py:_wrap_command``) and as an in-band stdout marker
        (``printf '\\n<marker>%s<marker>\\n' "$(pwd -P)"``).  Since
        ``_update_cwd`` runs *before* ``terminal_tool`` truncates the output,
        the marker is always present in ``result["output"]``, making the
        file read purely redundant I/O (~12 k file ops/hour at 2–3
        terminal calls/sec).

        ``_extract_cwd_from_output`` (below) already handles Windows
        MSYS-path normalisation, directory-existence validation, and marker
        stripping — identical to what the file read did, but without the
        extra I/O.
        """
