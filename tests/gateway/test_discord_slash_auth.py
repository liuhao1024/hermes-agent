...
@pytest.mark.asyncio
async def test_no_allowlist_allows_without_opt_in(adapter):
    """Without any access-control env vars, Discord traffic is allowed (backwards compat)."""
...
@pytest.mark.asyncio
async def test_no_allowlist_dm_allowed_without_opt_in(adapter):
    """DM slash commands also pass when no access control is configured."""
...
@pytest.mark.asyncio
async def test_channel_allowlist_configured_blocks_unmatched(adapter, monkeypatch):
    """When DISCORD_ALLOWED_CHANNELS is set but no allowlists match, traffic is denied."""
...
@pytest.mark.asyncio
async def test_missing_channel_id_allowed_without_allowlists(adapter):
    """No channel or user policy configured: allowed by default (backwards compat)."""
    interaction = _make_interaction("100200300", channel_id=None)
    assert await adapter._check_slash_authorization(interaction, "/help") is True
    interaction.response.send_message.assert_not_awaited()
